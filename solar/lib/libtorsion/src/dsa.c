...
#include <stdint.h>
#include <string.h>
#include "../include/torsion/drbg.h"
#include "../include/torsion/dsa.h"
#include "../include/torsion/hash.h"
#include "../include/torsion/mpi.h"
#include "../include/torsion/util.h"
#include "asn1.h"
#include "internal.h"
...
static int
dsa_group_generate(dsa_group_t *group, mp_bits_t bits,
...
dsa_group_is_sane(const dsa_group_t *group) {
  mp_bits_t pbits = mpz_bitlen(group->p);
  mp_bits_t qbits = mpz_bitlen(group->q);
  int ret = 0;
  mpz_t pm1;
...
fail:
  ...
  mpz_cleanse(s);
  dsa_priv_clear(&priv);
  torsion_memzero(&drbg, sizeof(drbg));
  torsion_memzero(&rng, sizeof(rng));
  torsion_memzero(bytes, sizeof(bytes));
  return ret;
}
